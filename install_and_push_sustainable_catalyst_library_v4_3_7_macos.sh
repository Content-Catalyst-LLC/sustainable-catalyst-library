#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="4.3.7"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Spotlight-Parity Hero Scale, Autoplay & Eight-Panel Disclosure"
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v437-validation-venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v437.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.7 release validation"
bash "$REPO/tests/run_v437_validation.sh"
...
printf 'Public Field Spotlights: Homepage Spotlight-scale Article Map lead, 14-second autoplay, pause/play, progress/status, and first-eight progressive disclosure.\n'
