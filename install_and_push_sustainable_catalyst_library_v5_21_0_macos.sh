...
RELEASE_ZIP="${2:-$BUNDLE_DIR/sustainable-catalyst-library-v5.21.0-repository.zip}"
TAG="v5.21.0"
TMP="$(mktemp -d /tmp/sc-library-v5210.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
SOURCE_DIR="$TMP/sustainable-catalyst-library-v5.21.0"
[[ -f "$SOURCE_DIR/library-backend/app/main.py" ]] || fail "repository payload missing backend"
[[ -f "$SOURCE_DIR/upgrade_library_backend_v2_32_0_contabo.sh" ]] || fail "v2.32.0 deployment installer missing"
cd "$SOURCE_DIR"
./tests/run_v5210_validation.sh
echo "=== SYNCHRONIZING v5.21.0 INTO GIT CHECKOUT ==="
...
git status --short
git add -A
if ! git diff --cached --quiet; then git commit -m "Library v5.21.0 Reproducible Visual Research Sessions & Workspace Handoff"; fi
if git rev-parse "$TAG" >/dev/null 2>&1; then
  [[ "$(git rev-list -n1 "$TAG")" == "$(git rev-parse HEAD)" ]] || fail "$TAG already exists and does not point to HEAD"
else
  git tag -a "$TAG" -m "Sustainable Catalyst Library v5.21.0 — Reproducible Visual Research Sessions & Workspace Handoff"
fi
git push origin HEAD
git push origin "$TAG"
echo "PASS: Knowledge Library v5.21.0 committed, tagged, and pushed."
