...
RELEASE_VERSION="4.3.15"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Unified Search & Guided Discovery"
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v4315-validation-venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4315.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.15 release validation"
bash "$REPO/tests/run_v4315_validation.sh"
