#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="5.6.1.1"
PLUGIN_VERSION="5.6.1.1"
RELEASE_NAME="Sustainable Catalyst Library v5.6.1.1 — Publications Spotlight Context Repair"
DOWNLOADS="${HOME}/Downloads"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPOSITORY_BASENAME="sustainable-catalyst-library-v5.6.1.1-repository.zip"
CHECKSUM_BASENAME="SHA256SUMS-v5.6.1.1.txt"
...
find_release_zip(){
  ...
  find "$DOWNLOADS" -maxdepth 3 -type f -name 'sustainable-catalyst-library-v5.6.1.1-repository*.zip' -print 2>/dev/null | sort | tail -1
}
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v5611-validation-venv"
...
[[ -f "$ZIP" ]] || fail "v5.6.1.1 repository ZIP was not found."
REPO="$(find_repo || true)"
...
BACKUP="$DOWNLOADS/sustainable-catalyst-library-before-v5.6.1.1-$(date +%Y%m%d-%H%M%S).zip"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v5611.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Installing v5.6.1.1 repository payload"
rsync -a --exclude='.git/' --exclude='.DS_Store' --exclude='__MACOSX/' --exclude='.venv/' --exclude='.pytest_cache/' --exclude='__pycache__/' "$SOURCE/" "$REPO/"

say "Running preservation + homepage-console + Publications spotlight validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v561_validation.sh"

grep -Fq 'Version: 5.6.1.1' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin version marker is not 5.6.1.1."
grep -Fq "SC_LIBRARY_VERSION', '5.6.1.1'" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "SC_LIBRARY_VERSION is not 5.6.1.1."
grep -Fq "public const SHORTCODE = 'sc_library_homepage_console'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-homepage-console.php" || fail "Homepage console shortcode is missing."
grep -Fq "'context' => 'library'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-homepage-spotlight.php" || fail "Spotlight context attribute is missing."
grep -Fq "'system_label' => __( 'Publications'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-homepage-spotlight.php" || fail "Publications spotlight presentation is missing."
grep -Fq 'SC_Library_Research_Network_Console::source_registry()' "$REPO/sustainable-catalyst-library/includes/class-sc-library-homepage-console.php" || fail "Homepage console does not reuse the Research Network source registry."
grep -Fq 'University College Dublin' "$REPO/sustainable-catalyst-library/includes/class-sc-library-research-network-console.php" || fail "University College Dublin direct-source definition is missing."
grep -Fq '[sc_library_homepage_console mode="full"]' "$REPO/HOMEPAGE_LIBRARY_WIDGET_v5.6.1.txt" || fail "Homepage insertion shortcode is missing."
grep -Fq 'R3.2.1 preserves the visible breadth restored in R3.' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R3.2.1.html" || fail "R3.2.1 page baseline is missing."
! grep -Fq 'primary-research-doors' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R3.2.1.html" || fail "Three Research Front Doors returned in the current page baseline."
grep -Fq '__version__ = "1.1.0"' "$REPO/library-backend/app/__init__.py" || fail "Python backend service is not v1.1.0."

if [[ -z "$(git -C "$REPO" status --porcelain)" ]]; then
  printf 'Repository already matches v5.6.1.1; nothing to commit.\n'
else
...
printf '\nPASS - Sustainable Catalyst Library v5.6.1.1 installed, validated, committed, and pushed.\n'
