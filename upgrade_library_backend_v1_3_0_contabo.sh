...
ZIP="${1:-/tmp/sustainable-catalyst-library-backend-v1.3.0.zip}"
[[ -f "$ZIP" ]] || { echo "ERROR: backend ZIP not found: $ZIP" >&2; exit 1; }
[[ -f "$LIVE/.env" ]] || { echo "ERROR: existing backend .env not found at $LIVE/.env" >&2; exit 1; }
ts="$(date +%Y%m%d-%H%M%S)"
tmp="$(mktemp -d /tmp/sc-library-backend-v130.XXXXXX)"
trap 'rm -rf "$tmp"' EXIT
env_backup="/tmp/sc-library-backend-v1.3.0.env.$ts"
app_backup="$BASE/library-backend.before-v1.3.0-$ts"
...
sources_json="$(curl -fsS http://127.0.0.1:8087/v1/biomedical-sources)"
printf '%s\n' "$health_json" | python3 -m json.tool
printf '%s\n' "$sources_json" | python3 -m json.tool
version="$(printf '%s' "$health_json" | python3 -c 'import json,sys; print(json.load(sys.stdin).get("version", ""))')"
[[ "$version" == "1.3.0" ]] || { echo "ERROR: expected backend v1.3.0, got: $version" >&2; exit 1; }
python3 -c 'import json,sys; d=json.load(sys.stdin); keys=[x.get("key") for x in d.get("sources",[])]; required=["pubmed","pmc","clinicaltrials","mesh","rxnorm"]; missing=[x for x in required if x not in keys]; assert not missing, f"missing biomedical sources: {missing}"' <<<"$sources_json"
printf '\nPASS - Library backend v1.3.0 upgraded and biomedical source registry is active.\n'
