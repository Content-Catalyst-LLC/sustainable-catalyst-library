...
ZIP="${1:-/tmp/sustainable-catalyst-library-backend-v1.5.0.zip}"
[[ -f "$ZIP" ]] || { echo "ERROR: backend ZIP not found: $ZIP" >&2; exit 1; }
[[ -f "$LIVE/.env" ]] || { echo "ERROR: existing backend .env not found at $LIVE/.env" >&2; exit 1; }
ts="$(date +%Y%m%d-%H%M%S)"
tmp="$(mktemp -d /tmp/sc-library-backend-v150.XXXXXX)"
trap 'rm -rf "$tmp"' EXIT
env_backup="/tmp/sc-library-backend-v1.5.0.env.$ts"
app_backup="$BASE/library-backend.before-v1.5.0-$ts"
...
term_json="$(curl -fsS http://127.0.0.1:8087/v1/medical-terminology)"
printf '%s\n' "$health_json" | python3 -m json.tool
printf '%s\n' "$term_json" | python3 -m json.tool
version="$(printf '%s' "$health_json" | python3 -c 'import json,sys; print(json.load(sys.stdin).get("version", ""))')"
[[ "$version" == "1.5.0" ]] || { echo "ERROR: expected backend v1.5.0, got: $version" >&2; exit 1; }
python3 -c 'import json,sys; d=json.load(sys.stdin); names=[x.get("key") for x in d.get("sources",[]) if isinstance(x,dict)]; assert "icd11" in names and "mesh" in names and "rxnorm" in names, names' <<<"$term_json"
printf '\nPASS - Library backend v1.5.0 upgraded; ICD-11/MeSH/RxNorm terminology manifest is active.\n'
