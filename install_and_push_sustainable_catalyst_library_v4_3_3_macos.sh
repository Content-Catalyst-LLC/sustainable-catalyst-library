#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="4.3.3"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Dynamic Publications Spotlight, Field Deck & Editorial Customization"
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v433-validation-venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v433.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.3 release validation"
bash "$REPO/tests/run_v433_validation.sh"
...
printf 'Admin customization: SC Library -> Publications\n'
printf 'Public contract: 14-field dynamic Spotlight deck; one active Article Map hero + up to four publications; no Blog Roll.\n'
