...
RELEASE_VERSION="4.3.21"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Course Access Intelligence & Learning Pathways"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v4321-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4321.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.21 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v4321_validation.sh"

grep -q 'Version: 4.3.21' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Installed plugin version marker is not v4.3.21."
...
printf 'Research Library page replacement: RESEARCH_LIBRARY_PAGE_v4.3.21.html\n'
printf 'After plugin deployment, replace the Research Library page body with the v4.3.21 page artifact and clear caches.\n'
