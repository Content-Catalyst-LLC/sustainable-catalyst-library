...
PYTHONPATH="$ROOT/library-backend" "$PY" -m pytest -q \
  library-backend/tests/test_biomedical_evidence_graph_v190.py \
  tests/test_biomedical_evidence_graph_reliability_v591.py
...
php -l sustainable-catalyst-library/includes/class-sc-library-biomedical-evidence-graph.php >/dev/null
node --check sustainable-catalyst-library/assets/js/sc-library-biomedical-evidence-graph-v591.js
printf '%s\n' 'PASS: Sustainable Catalyst Library v5.9.1 Biomedical Evidence Graph Reliability & Provenance Repair validation'
