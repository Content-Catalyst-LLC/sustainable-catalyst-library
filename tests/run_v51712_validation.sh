...
echo "=== Knowledge Library v5.17.1.2 / backend v2.28.3 validation ==="
echo "Python: $($PY --version 2>&1)"
grep -q 'Version: 5.17.1.2' "$ROOT/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "WordPress version mismatch"
grep -q 'Stable tag: 5.17.1.2' "$ROOT/sustainable-catalyst-library/readme.txt" || fail "WordPress stable tag mismatch"
grep -q '__version__ = "2.28.3"' "$ROOT/library-backend/app/__init__.py" || fail "backend version mismatch"
...
grep -q '\-o "$TMP/corpus.json"' "$ROOT/upgrade_library_backend_v2_28_3_contabo.sh" || fail "corpus response is not file-backed"
grep -q '< "$TMP/corpus.json"' "$ROOT/upgrade_library_backend_v2_28_3_contabo.sh" || fail "corpus JSON is not streamed through stdin"
! grep -q '"$corpus"' "$ROOT/upgrade_library_backend_v2_28_3_contabo.sh" || fail "large corpus JSON is still passed through argv"
! grep -q 'python3 -m json.tool | head' "$ROOT/upgrade_library_backend_v2_28_3_contabo.sh" || fail "unsafe head/pipefail pipeline present"
"$PY" -m compileall -q "$ROOT/library-backend/app"
if "$PY" -c 'import pytest' >/dev/null 2>&1; then
  PYTHONPATH="$ROOT/library-backend" "$PY" -m pytest -q \
    "$ROOT/tests/test_corpus_validator_argument_length_repair_v51712.py" \
...
bash -n "$ROOT/upgrade_library_backend_v2_28_3_contabo.sh"
bash -n "$ROOT/install_and_push_sustainable_catalyst_library_v5_17_1_2_macos.sh"
echo "PASS: Knowledge Library v5.17.1.2 Corpus Validator Argument-Length Repair validation complete"
