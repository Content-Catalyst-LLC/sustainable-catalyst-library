...
set -euo pipefail
cd "$(dirname "$0")/.."
printf 'Sustainable Catalyst Library v4.3.0 validation\n\n'
grep -q 'Version: 4.3.0' sustainable-catalyst-library/sustainable-catalyst-library.php
grep -q "SC_LIBRARY_VERSION', '4.3.0" sustainable-catalyst-library/sustainable-catalyst-library.php
grep -q 'Stable tag: 4.3.0' sustainable-catalyst-library/readme.txt
printf 'PASS: release version markers\n'
python3 -m pytest -q tests/test_publications_v430.py
...
printf 'PASS: v4.2.0 Spotlight module preserved behind v4.3.0 plugin release\n'
find sustainable-catalyst-library -name '*.php' -print0 | xargs -0 -n1 php -l >/dev/null
printf 'PASS: PHP syntax\n'
printf '\nPASS: v4.3.0 Publications foundation validation completed.\n'
