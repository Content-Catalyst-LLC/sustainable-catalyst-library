...
set -euo pipefail
cd "$(dirname "$0")/.."
printf 'Sustainable Catalyst Library v4.1.1 validation\n\n'
grep -q 'Version: 4.1.1' sustainable-catalyst-library/sustainable-catalyst-library.php
grep -q "SC_LIBRARY_VERSION', '4.1.1" sustainable-catalyst-library/sustainable-catalyst-library.php
grep -q 'Stable tag: 4.1.1' sustainable-catalyst-library/readme.txt
printf 'PASS: release version markers\n'
if python3 -c 'import pytest' >/dev/null 2>&1; then
  python3 -m pytest -q tests/test_homepage_spotlight_v410.py tests/test_homepage_spotlight_source_search_v411.py
else
...
printf '\nPASS: v4.1.1 source discovery repair validation completed.\n'
