...
echo "=== Knowledge Library v5.17.1 / backend v2.28.1 validation ==="
echo "Python: $($PY --version 2>&1)"
grep -q 'Version: 5.17.1' "$ROOT/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "WordPress version mismatch"
grep -q 'Stable tag: 5.17.1' "$ROOT/sustainable-catalyst-library/readme.txt" || fail "WordPress stable tag mismatch"
grep -q '__version__ = "2.28.1"' "$ROOT/library-backend/app/__init__.py" || fail "backend version mismatch"
grep -q 'CORPUS_KNOWLEDGE_MAP_CONTRACT = "sc-library-publication-corpus-knowledge-map/1.0"' "$ROOT/library-backend/app/publication_corpus_maps.py" || fail "corpus contract missing"
grep -q '@app.get("/v1/publication-knowledge-maps/corpus")' "$ROOT/library-backend/app/main.py" || fail "corpus route missing"
grep -q "'scope' => 'corpus'" "$ROOT/sustainable-catalyst-library/includes/class-sc-library-knowledge-landscape.php" || fail "shortcode does not default to corpus mode"
grep -q "'source_key' => 'wordpress-main'" "$ROOT/sustainable-catalyst-library/includes/class-sc-library-knowledge-landscape.php" || fail "wordpress-main corpus source missing"
grep -q 'publication-topic-cooccurrence' "$ROOT/library-backend/app/publication_corpus_maps.py" || fail "corpus topic relationship missing"
grep -q 'embedding-cosine-similarity' "$ROOT/library-backend/app/publication_corpus_maps.py" || fail "semantic relationship contract missing"
...
echo "PASS: Knowledge Library v5.17.1 Publication Corpus Integration validation complete"
