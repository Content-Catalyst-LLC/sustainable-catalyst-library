...
node --check "$ROOT/sustainable-catalyst-library/assets/js/sc-library-capability-hub-v560r1.js" >/dev/null
echo "==> Python compile"
"$PYTHON_BIN" -m compileall -q "$ROOT/library-backend/app" "$ROOT/library-backend/tests"
echo "==> v5.6.0 R1 preservation and Explorer regression tests"
...
echo "PASS - Sustainable Catalyst Library v5.6.0 R1 validation"
