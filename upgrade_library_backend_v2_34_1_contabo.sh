#!/usr/bin/env bash
set -Eeuo pipefail
ARCHIVE="${1:-/tmp/sustainable-catalyst-library-backend-v2.34.1.zip}"
ROOT=/opt/sustainable-catalyst/library-backend
BACKUP_ROOT=/opt/sustainable-catalyst/backups
TMP="$(mktemp -d /tmp/sc-library-v2341.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
SRC="$TMP/sustainable-catalyst-library-backend-v2.34.1"
[[ -f "$SRC/app/__init__.py" ]] || fail "backend payload missing"
grep -q '__version__ = "2.34.1"' "$SRC/app/__init__.py" || fail "payload is not backend v2.34.1"
...
if [[ -d "$ROOT" ]]; then tar -C "$(dirname "$ROOT")" -czf "$BACKUP_ROOT/library-backend-before-v2.34.1-$stamp.tgz" "$(basename "$ROOT")"; fi
...
echo "PASS: Library backend v2.34.1 Local Validation Environment Repair deployed."
