...
RELEASE_VERSION="4.3.29"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Saved Searches, Watchlists & Research Queue"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v4329-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
[[ -f "$ZIP" ]] || fail "v4.3.29 repository ZIP was not found."
REPO="$(find_repo || true)"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4329.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.29 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v4329_validation.sh"
...
PAGE="$REPO/RESEARCH_LIBRARY_PAGE_v4.3.29.html"

say "Checking release boundaries"
grep -q 'Version: 4.3.29' "$MAIN" || fail "Plugin version marker is not v4.3.29."
grep -q "SC_LIBRARY_VERSION', '4.3.29'" "$MAIN" || fail "SC_LIBRARY_VERSION is not v4.3.29."
...
grep -q 'MODULE_COUNT = 34' "$BOOT" || fail "Extension module count is not 34."
grep -q 'class-sc-library-personal-collections-recommendations.php' "$BOOT" || fail "Personal Library module is not registered."
grep -q 'class-sc-library-saved-searches-watchlists-queue.php' "$BOOT" || fail "Research continuity module is not registered."
...
grep -q "VERSION = '4.3.29'" "$CONTINUITY" || fail "Research continuity module version is not v4.3.29."
...
grep -q "REST_ROUTE = '/research-continuity'" "$CONTINUITY" || fail "Research continuity API route is missing."
grep -q "'background_monitoring'     => false" "$CONTINUITY" || fail "Passive-watchlist background-monitoring boundary is missing."
grep -q "'automatic_notifications'   => false" "$CONTINUITY" || fail "Passive-watchlist notification boundary is missing."
grep -q "public const VERSION = '4.3.29'" "$ROUTE" || fail "Route/identity health runtime is not version-aligned."
grep -q "'personal_library'   => 'sc_library_personal_items_v4328'" "$ROUTE" || fail "Account continuity does not include My Library."
grep -q "'saved_searches'     => 'sc_library_saved_searches_v4329'" "$ROUTE" || fail "Account continuity does not include saved searches."
grep -q "'watchlists'         => 'sc_library_watchlists_v4329'" "$ROUTE" || fail "Account continuity does not include watchlists."
grep -q "'research_queue'     => 'sc_library_research_queue_v4329'" "$ROUTE" || fail "Account continuity does not include the research queue."
...
[[ -f "$PAGE" ]] || fail "Research Library v4.3.29 page artifact is missing."
grep -q '\[sc_personal_library title="My Library"\]' "$PAGE" || fail "Research Library page does not expose My Library."
grep -q '\[sc_research_continuity title="Saved Research & Queue"\]' "$PAGE" || fail "Research Library page does not expose Saved Research & Queue."
...
say "Complete"
printf 'Installed Sustainable Catalyst Library v%s — Saved Searches, Watchlists & Research Queue\n' "$RELEASE_VERSION"
printf 'WordPress plugin ZIP: %s\n' "$SCRIPT_DIR/$PLUGIN_BASENAME"
printf 'Research Library page replacement: %s\n' "$SCRIPT_DIR/RESEARCH_LIBRARY_PAGE_v4.3.29.html"
...
printf 'Signed-in research continuity API: https://sustainablecatalyst.com/wp-json/sc-library/v1/research-continuity\n'
