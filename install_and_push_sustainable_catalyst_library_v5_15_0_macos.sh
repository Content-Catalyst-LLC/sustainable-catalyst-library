...
RELEASE_ZIP="${2:-$BUNDLE_DIR/sustainable-catalyst-library-v5.15.0-repository.zip}"
TAG="v5.15.0"
TMP="$(mktemp -d /tmp/sc-library-v5150.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
SOURCE_DIR="$TMP/sustainable-catalyst-library-v5.15.0"
[[ -f "$SOURCE_DIR/library-backend/app/research_extraction.py" ]] || fail "repository payload missing research extraction"
cd "$SOURCE_DIR"
./tests/run_v5150_validation.sh
echo "=== SYNCHRONIZING v5.15.0 INTO GIT CHECKOUT ==="
...
if git diff --cached --quiet; then
  ...
  git commit -m "Library v5.15.0 Entity, Finding & Claim Extraction"
fi
if git rev-parse "$TAG" >/dev/null 2>&1; then
  target="$(git rev-list -n1 "$TAG")"; head="$(git rev-parse HEAD)"
  [[ "$target" == "$head" ]] || fail "$TAG already exists and does not point to HEAD"
else
  git tag -a "$TAG" -m "Sustainable Catalyst Library v5.15.0 — Entity, Finding & Claim Extraction"
fi
git push origin HEAD
git push origin "$TAG"
echo "PASS: Knowledge Library v5.15.0 committed, tagged, and pushed."
