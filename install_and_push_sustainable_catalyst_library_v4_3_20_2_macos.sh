...
RELEASE_VERSION="4.3.20.2"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Knowledge Pathway Editorial Index & Course Catalog Refinement"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v43202-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v43202.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.20.2 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v43202_validation.sh"
...
printf 'Research Library page replacement: RESEARCH_LIBRARY_PAGE_v4.3.20.2.html\n'
printf 'After plugin deployment, replace the Research Library page body with the v4.3.20.2 page artifact and clear caches.\n'
