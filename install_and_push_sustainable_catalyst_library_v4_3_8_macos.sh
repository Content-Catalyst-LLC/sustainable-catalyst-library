#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="4.3.8"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Field Spotlight Visual Refinement, Editorial Density & Control Simplification"
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v438-validation-venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v438.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.8 release validation"
bash "$REPO/tests/run_v438_validation.sh"
...
printf 'Public Field Spotlights: flatter eight-panel rail, Homepage Spotlight-scale Article Map lead, thumbnail editorial rows, quiet transport controls, and integrated additional-fields disclosure.\n'
