#!/usr/bin/env bash
set -Eeuo pipefail
ARCHIVE="${1:-/tmp/sustainable-catalyst-library-backend-v2.35.1.zip}"
ROOT=/opt/sustainable-catalyst/library-backend
BACKUP_ROOT=/opt/sustainable-catalyst/backups
TMP="$(mktemp -d /tmp/sc-library-v2351.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
SRC="$TMP/sustainable-catalyst-library-backend-v2.35.1"
[[ -f "$SRC/app/__init__.py" ]] || fail "backend payload missing"
grep -q '__version__ = "2.35.1"' "$SRC/app/__init__.py" || fail "payload is not backend v2.35.1"
...
if [[ -d "$ROOT" ]]; then tar -C "$(dirname "$ROOT")" -czf "$BACKUP_ROOT/library-backend-before-v2.35.1-$stamp.tgz" "$(basename "$ROOT")"; fi
...
python3 -c 'import json,sys; d=json.load(sys.stdin); print(json.dumps({"ok":d.get("ok"),"version":d.get("version"),"database":d.get("database")},indent=2)); assert d.get("ok") is True; assert d.get("version")=="2.35.1"; c=d.get("capabilities",{}); assert c.get("release_certification_alignment") is True; assert c.get("publication_cross_publication_evidence_synthesis") is True; assert c.get("publication_support_connected_structures") is True; assert c.get("publication_explicit_competing_hypotheses") is True; assert c.get("publication_argument_path_visualization") is True' < "$TMP/health.json"
...
echo "PASS: Library backend v2.35.1 Release Certification & Readiness Alignment Repair deployed."
