...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Spotlight-Parity Publications Editorial Boards"
...
ensure_pytest(){
  ...
  # Keep the validation environment OUTSIDE the repository because rsync --delete
  # intentionally replaces repository contents during installation.
  tool_venv="$DOWNLOADS/.sc-library-v432-validation-venv"
  say "Preparing isolated validation environment"
  if [[ ! -x "$tool_venv/bin/python3" ]]; then
...
(cd "$(dirname "$REPO")" && zip -qry "$BACKUP" "$(basename "$REPO")" -x '*/.git/*' '*/.DS_Store' '*/__MACOSX/*')
printf 'Safety backup: %s\n' "$BACKUP"
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v432.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
rsync -a --delete --exclude='.git/' --exclude='.DS_Store' --exclude='__MACOSX/' "$SOURCE/" "$REPO/"
say "Running v4.3.2 release validation"
bash "$REPO/tests/run_v432_validation.sh"
...
say "Complete"
printf 'Installed %s\n' "$RELEASE_NAME"
printf 'Publications shortcode: [sc_publications]\n'
printf 'Editorial contract: Spotlight-parity five-row boards: Article Map lead row + up to four full-width publications; no Blog Roll.\n'
