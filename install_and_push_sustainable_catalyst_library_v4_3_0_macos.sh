...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Publications Foundation & Article Map Editorial System"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v430.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.0 release validation"
bash "$REPO/tests/run_v430_validation.sh"
...
printf 'Editorial contract: Article Map hero + four curated articles; no Blog Roll.\n'
