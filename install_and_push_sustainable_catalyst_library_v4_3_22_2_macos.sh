...
RELEASE_VERSION="4.3.22.2"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Publications Fail-Open Field Switching & Runtime Hardening"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v43222-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v43222.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.22.2 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v43222_validation.sh"

grep -q 'Version: 4.3.22.2' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Installed plugin version marker is not v4.3.22.2."
grep -q "public const VERSION = '4.3.22.2'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-publications.php" || fail "Core Publications runtime was not upgraded to v4.3.22.2."
grep -q 'data-sc-publications="v4.3.22.2"' "$REPO/sustainable-catalyst-library/templates/publications.php" || fail "Core Publications template runtime mismatch remains."
grep -q 'data-sc-field-spotlights="v4.3.22.2"' "$REPO/sustainable-catalyst-library/templates/field-spotlights.php" || fail "Field Spotlight master runtime mismatch remains."
grep -q 'prevent native navigation only after a verified switch' "$REPO/sustainable-catalyst-library/assets/js/sc-library-publications.js" || fail "Core fail-open field-switch contract is missing."
grep -q 'only suppress the server fallback after a verified in-place field switch' "$REPO/sustainable-catalyst-library/assets/js/sc-library-field-spotlights.js" || fail "Field Spotlight fail-open field-switch contract is missing."
