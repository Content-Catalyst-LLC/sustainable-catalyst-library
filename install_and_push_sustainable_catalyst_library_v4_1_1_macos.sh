...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Homepage Spotlight Source Discovery Repair"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v411.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.1.1 release validation"
grep -q 'Version: 4.1.1' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin header version does not match 4.1.1."
grep -q "SC_LIBRARY_VERSION', '4.1.1" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin constant does not match 4.1.1."
grep -q 'Stable tag: 4.1.1' "$REPO/sustainable-catalyst-library/readme.txt" || fail "WordPress stable tag does not match 4.1.1."
bash "$REPO/tests/run_v411_validation.sh"
...
if [[ -z "$(git -C "$REPO" status --porcelain)" ]]; then
  ...
  git -C "$REPO" add -A
  git -C "$REPO" commit -m "Sustainable Catalyst Library v${RELEASE_VERSION} — Homepage Spotlight Source Discovery Repair"
fi
