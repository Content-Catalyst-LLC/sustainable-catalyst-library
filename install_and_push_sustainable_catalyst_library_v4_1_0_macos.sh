...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Curated Homepage Spotlight"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v410.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.1.0 release validation"
grep -q 'Version: 4.1.0' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin header version does not match 4.1.0."
grep -q "SC_LIBRARY_VERSION', '4.1.0" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin constant does not match 4.1.0."
grep -q 'Stable tag: 4.1.0' "$REPO/sustainable-catalyst-library/readme.txt" || fail "WordPress stable tag does not match 4.1.0."
bash "$REPO/tests/run_v410_validation.sh"
...
if [[ -z "$(git -C "$REPO" status --porcelain)" ]]; then
  ...
  git -C "$REPO" add -A
  git -C "$REPO" commit -m "Sustainable Catalyst Library v${RELEASE_VERSION} — Curated Homepage Spotlight"
fi
