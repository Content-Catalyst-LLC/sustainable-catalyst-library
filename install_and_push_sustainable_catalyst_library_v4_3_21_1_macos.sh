...
RELEASE_VERSION="4.3.21.1"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Publications Runtime Recovery & Multi-Panel Resilience"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v43211-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v43211.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.21.1 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v43211_validation.sh"

grep -q 'Version: 4.3.21.1' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Installed plugin version marker is not v4.3.21.1."
grep -q "public const VERSION = '4.3.21.1'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-field-spotlights.php" || fail "Field Spotlight runtime version is not v4.3.21.1."
...
printf 'No Research Library or Publications page HTML replacement is required.\n'
printf 'After WordPress plugin deployment, purge page/object/CDN caches and hard-refresh Publications so the v4.3.21.1 Field Spotlight assets are fetched.\n'
