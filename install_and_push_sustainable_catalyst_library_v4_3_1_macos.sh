...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Full Article Map Registry & Publications Topic Coverage"
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v431-validation-venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v431.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.1 release validation"
bash "$REPO/tests/run_v431_validation.sh"
...
printf 'Editorial contract: Article Map hero + four companion publications across the full registry; no Blog Roll.\n'
