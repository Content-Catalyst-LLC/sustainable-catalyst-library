#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="5.6.0-R2"
PLUGIN_VERSION="5.6.0.2"
RELEASE_NAME="Sustainable Catalyst Library v5.6.0 R2 — Featured Library Access, Research Librarian & Interface Density"
DOWNLOADS="${HOME}/Downloads"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPOSITORY_BASENAME="sustainable-catalyst-library-v5.6.0-R2-repository.zip"
CHECKSUM_BASENAME="SHA256SUMS-v5.6.0-R2.txt"
...
find_release_zip(){
  ...
  find "$DOWNLOADS" -maxdepth 3 -type f -name 'sustainable-catalyst-library-v5.6.0-R2-repository*.zip' -print 2>/dev/null | sort | tail -1
}
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v560r2-validation-venv"
...
[[ -f "$ZIP" ]] || fail "v5.6.0 R2 repository ZIP was not found."
REPO="$(find_repo || true)"
...
BACKUP="$DOWNLOADS/sustainable-catalyst-library-before-v5.6.0-R2-$(date +%Y%m%d-%H%M%S).zip"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v560r2.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Installing v5.6.0 R2 repository payload"
...
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v560r2_validation.sh"

grep -Fq 'Version: 5.6.0.2' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin version marker is not 5.6.0.2."
grep -Fq "SC_LIBRARY_VERSION', '5.6.0.2'" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "SC_LIBRARY_VERSION is not 5.6.0.2."
...
grep -Fq '[sc_library_capability_hub' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R2.html" || fail "R2 page does not include the capability hub."
grep -Fq '"protected_shortcodes"' "$REPO/LIBRARY_CAPABILITY_MANIFEST_v5.6.0-R2.json" || fail "Capability preservation manifest is missing."
grep -Fq 'featured-library-access' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R2.html" || fail "Featured Library Access front door is missing."
grep -Fq 'featured-research-librarian' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R2.html" || fail "Featured Research Librarian front door is missing."
grep -Fq 'height:min(72vh,760px)' "$REPO/sustainable-catalyst-library/assets/css/sc-library-capability-hub-v560r2.css" || fail "Bounded capability workspace CSS is missing."
grep -Fq '__version__ = "1.1.0"' "$REPO/library-backend/app/__init__.py" || fail "Python backend service is not v1.1.0."

if [[ -z "$(git -C "$REPO" status --porcelain)" ]]; then
  printf 'Repository already matches v5.6.0 R2; nothing to commit.\n'
else
...
printf '\nPASS - Sustainable Catalyst Library v5.6.0 R2 installed, validated, committed, and pushed.\n'
