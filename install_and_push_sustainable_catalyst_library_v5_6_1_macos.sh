...
RELEASE_NAME="Sustainable Catalyst Library v5.6.1 — Homepage Research Network & Knowledge Discovery Console"
DOWNLOADS="${HOME}/Downloads"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPOSITORY_BASENAME="sustainable-catalyst-library-v5.6.1-repository.zip"
CHECKSUM_BASENAME="SHA256SUMS-v5.6.1.txt"
...
find_release_zip(){
  ...
  find "$DOWNLOADS" -maxdepth 3 -type f -name 'sustainable-catalyst-library-v5.6.1-repository*.zip' -print 2>/dev/null | sort | tail -1
}
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v561-validation-venv"
...
[[ -f "$ZIP" ]] || fail "v5.6.1 repository ZIP was not found."
REPO="$(find_repo || true)"
...
BACKUP="$DOWNLOADS/sustainable-catalyst-library-before-v5.6.1-$(date +%Y%m%d-%H%M%S).zip"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v561.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Installing v5.6.1 repository payload"
rsync -a --exclude='.git/' --exclude='.DS_Store' --exclude='__MACOSX/' --exclude='.venv/' --exclude='.pytest_cache/' --exclude='__pycache__/' "$SOURCE/" "$REPO/"

say "Running preservation + homepage-console validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v561_validation.sh"

grep -Fq 'Version: 5.6.1' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin version marker is not 5.6.1."
grep -Fq "SC_LIBRARY_VERSION', '5.6.1'" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "SC_LIBRARY_VERSION is not 5.6.1."
...
if [[ -z "$(git -C "$REPO" status --porcelain)" ]]; then
  printf 'Repository already matches v5.6.1; nothing to commit.\n'
else
...
printf '\nPASS - Sustainable Catalyst Library v5.6.1 installed, validated, committed, and pushed.\n'
