#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="4.3.35"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Access Intelligence II"
...
ensure_pytest(){ local c base_py venv; say "Preparing isolated validation environment"; for c in "${SC_LIBRARY_VALIDATION_PYTHON:-}" "$DOWNLOADS/.sc-library-v4335-validation-venv/bin/python3" "$DOWNLOADS/.sc-library-v4333-validation-venv/bin/python3" "$(command -v python3 2>/dev/null || true)"; do [[ -n "$c" && -x "$c" ]] || continue; if "$c" -m pytest --version >/dev/null 2>&1; then export SC_LIBRARY_VALIDATION_PYTHON="$c"; printf 'pytest ready: %s\n' "$c"; return; fi; done; base_py="$(command -v python3 2>/dev/null || true)"; [[ -n "$base_py" ]] || fail "Python 3 is required."; venv="$DOWNLOADS/.sc-library-v4335-validation-venv"; rm -rf "$venv"; "$base_py" -m venv "$venv"; "$venv/bin/python3" -m pip install -q --upgrade pip; "$venv/bin/python3" -m pip install -q 'pytest>=8'; export SC_LIBRARY_VALIDATION_PYTHON="$venv/bin/python3"; }
ZIP="$(find_release_zip)"; [[ -f "$ZIP" ]] || fail "v4.3.35 repository ZIP was not found."
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4335.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT; unzip -q "$ZIP" -d "$TMP"; MAIN_FILE="$(find "$TMP" -type f -path '*/sustainable-catalyst-library/sustainable-catalyst-library.php' -print | head -1)"; [[ -f "$MAIN_FILE" ]] || fail "Could not locate plugin entry point."; SOURCE="$(dirname "$(dirname "$MAIN_FILE")")"
...
say "Running v4.3.35 validation"; SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v4335_validation.sh"
MAIN="$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php"; BOOT="$REPO/sustainable-catalyst-library/includes/class-sc-library-extension-bootstrap-v402.php"; ROUTE="$REPO/sustainable-catalyst-library/includes/class-sc-library-canonical-route-identity.php"; MOD="$REPO/sustainable-catalyst-library/includes/class-sc-library-access-intelligence-ii.php"; OLD_ACCESS="$REPO/sustainable-catalyst-library/includes/class-sc-library-research-librarian-access-intelligence.php"; PUBLIC="$REPO/sustainable-catalyst-library/includes/class-sc-library-public-library-network.php"; INST="$REPO/sustainable-catalyst-library/includes/class-sc-library-institutional-connector-expansion.php"; META="$REPO/sustainable-catalyst-library/includes/class-sc-library-metadata-quality-entity-resolution.php"; STACK="$REPO/sustainable-catalyst-library/templates/field-spotlights.php"; PAGE="$REPO/RESEARCH_LIBRARY_PAGE_v4.3.35.html"
say "Checking release boundaries"
grep -q 'Version: 4.3.35' "$MAIN" || fail "Plugin version marker is not v4.3.35."
grep -q "SC_LIBRARY_VERSION', '4.3.35'" "$MAIN" || fail "SC_LIBRARY_VERSION is not v4.3.35."
grep -q 'MODULE_COUNT = 40' "$BOOT" || fail "Extension module count is not 40."
grep -q 'class-sc-library-access-intelligence-ii.php' "$BOOT" || fail "Access Intelligence II module is not registered."
grep -q "VERSION = '4.3.35'" "$MOD" || fail "Access Intelligence II module version is not v4.3.35."
grep -q "REST_ROUTE = '/access-intelligence-v2'" "$MOD" || fail "Access Intelligence II REST route is missing."
grep -q "VERSION = '4.3.24'" "$OLD_ACCESS" || fail "v4.3.24 Access Intelligence classifier was not preserved."
grep -q "MY_LIBRARIES_META = 'sc_library_my_libraries_v4319'" "$MOD" || fail "My Libraries continuity contract is missing."
grep -q "catalog_search_is_not_a_holding' => true" "$MOD" || fail "Catalog-search boundary is missing."
grep -q "holding_is_not_user_eligibility' => true" "$MOD" || fail "Holding/eligibility boundary is missing."
grep -q "automatic_access_claim' => false" "$MOD" || fail "No automatic access-claim boundary is missing."
grep -q "resolve_search_url( 'worldcat'" "$MOD" || fail "WorldCat fallback is missing."
grep -q "resolve_search_url( 'loc'" "$MOD" || fail "Library of Congress fallback is missing."
grep -q 'public static function registry()' "$PUBLIC" || fail "Public Library registry was not preserved."
grep -q 'public static function registry()' "$INST" || fail "Institutional Connector registry was not preserved."
...
grep -q '\[sc_access_intelligence_ii title="Access Intelligence II"\]' "$PAGE" || fail "Research Library page is missing Access Intelligence II."
grep -q 'data-sc-field-stack="v4.3.22.4"' "$STACK" || fail "Publications field stack was not preserved."
say "Checking v4.3.34 -> v4.3.35 upgrade delta"; DELTA_COUNT="$(git -C "$REPO" status --porcelain | wc -l | tr -d ' ')"; [[ "$DELTA_COUNT" == "16" ]] || fail "Expected a 16-file v4.3.35 delta; found $DELTA_COUNT files."; [[ -z "$(git -C "$REPO" status --porcelain | awk '$1 ~ /^D/ {print}')" ]] || fail "v4.3.35 unexpectedly deletes tracked files."
