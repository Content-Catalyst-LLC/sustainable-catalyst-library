#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="5.6.0-R1"
PLUGIN_VERSION="5.6.0.1"
RELEASE_NAME="Sustainable Catalyst Library v5.6.0 R1 — Capability-Preserving Dynamic Library Interface"
DOWNLOADS="${HOME}/Downloads"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPOSITORY_BASENAME="sustainable-catalyst-library-v5.6.0-R1-repository.zip"
CHECKSUM_BASENAME="SHA256SUMS-v5.6.0-R1.txt"
...
find_release_zip(){
  ...
  find "$DOWNLOADS" -maxdepth 3 -type f -name 'sustainable-catalyst-library-v5.6.0-R1-repository*.zip' -print 2>/dev/null | sort | tail -1
}
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v560r1-validation-venv"
...
[[ -f "$ZIP" ]] || fail "v5.6.0 R1 repository ZIP was not found."
REPO="$(find_repo || true)"
...
BACKUP="$DOWNLOADS/sustainable-catalyst-library-before-v5.6.0-R1-$(date +%Y%m%d-%H%M%S).zip"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v560r1.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Installing v5.6.0 R1 repository payload"
...
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v560r1_validation.sh"

grep -Fq 'Version: 5.6.0.1' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin version marker is not 5.6.0.1."
grep -Fq "SC_LIBRARY_VERSION', '5.6.0.1'" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "SC_LIBRARY_VERSION is not 5.6.0.1."
...
grep -Fq '[sc_library_capability_hub' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R1.html" || fail "R1 page does not include the capability hub."
grep -Fq '"protected_shortcodes"' "$REPO/LIBRARY_CAPABILITY_MANIFEST_v5.6.0-R1.json" || fail "Capability preservation manifest is missing."
grep -Fq '__version__ = "1.1.0"' "$REPO/library-backend/app/__init__.py" || fail "Python backend service is not v1.1.0."

if [[ -z "$(git -C "$REPO" status --porcelain)" ]]; then
  printf 'Repository already matches v5.6.0 R1; nothing to commit.\n'
else
...
printf '\nPASS - Sustainable Catalyst Library v5.6.0 R1 installed, validated, committed, and pushed.\n'
