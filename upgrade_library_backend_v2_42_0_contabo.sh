#!/usr/bin/env bash
set -Eeuo pipefail
ARCHIVE="${1:-/tmp/sustainable-catalyst-library-backend-v2.42.0.zip}"
ROOT=/opt/sustainable-catalyst/library-backend
BACKUP_ROOT=/opt/sustainable-catalyst/backups
TMP="$(mktemp -d /tmp/sc-library-v2420.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
SRC="$TMP/sustainable-catalyst-library-backend-v2.42.0"
[[ -f "$SRC/app/__init__.py" ]] || fail "backend payload missing"
grep -q '__version__ = "2.42.0"' "$SRC/app/__init__.py" || fail "payload is not backend v2.42.0"
grep -q 'sc-library-research-gap-novelty/1.0' "$SRC/app/research_gap_novelty.py" || fail "gap/novelty contract missing"
grep -q 'gap_signal_proves_global_absence' "$SRC/app/research_gap_novelty.py" || fail "gap guardrail missing"
...
if [[ -d "$ROOT" ]]; then tar -C "$(dirname "$ROOT")" -czf "$BACKUP_ROOT/library-backend-before-v2.42.0-$stamp.tgz" "$(basename "$ROOT")"; fi
...
assert d.get('ok') is True and d.get('version')=='2.42.0'
...
PY
echo "PASS: Library backend v2.42.0 Research Gap & Novelty Discovery deployed and verified."
