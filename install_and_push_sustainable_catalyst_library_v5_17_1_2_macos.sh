...
RELEASE_ZIP="${2:-$BUNDLE_DIR/sustainable-catalyst-library-v5.17.1.2-repository.zip}"
TAG="v5.17.1.2"
TMP="$(mktemp -d /tmp/sc-library-v51712.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
SOURCE_DIR="$TMP/sustainable-catalyst-library-v5.17.1.2"
[[ -f "$SOURCE_DIR/library-backend/app/publication_corpus_maps.py" ]] || fail "repository payload missing publication corpus engine"
[[ -f "$SOURCE_DIR/upgrade_library_backend_v2_28_3_contabo.sh" ]] || fail "v2.28.3 deployment installer missing"
cd "$SOURCE_DIR"
./tests/run_v51712_validation.sh
echo "=== SYNCHRONIZING v5.17.1.2 INTO GIT CHECKOUT ==="
...
if git diff --cached --quiet; then
  ...
  git commit -m "Library v5.17.1.2 Corpus Validator Argument-Length Repair"
fi
if git rev-parse "$TAG" >/dev/null 2>&1; then
  target="$(git rev-list -n1 "$TAG")"; head="$(git rev-parse HEAD)"
  [[ "$target" == "$head" ]] || fail "$TAG already exists and does not point to HEAD"
else
  git tag -a "$TAG" -m "Sustainable Catalyst Library v5.17.1.2 — Corpus Validator Argument-Length Repair"
fi
git push origin HEAD
git push origin "$TAG"
echo "PASS: Knowledge Library v5.17.1.2 committed, tagged, and pushed."
