...
if ! grep -q "Whiteboards and Chalkboards" sustainable-catalyst-library/readme.txt; then
  echo "ERROR: Whiteboard and Chalkboard release marker is missing."
  exit 1
fi
for required in \
  sustainable-catalyst-library/includes/class-sc-library-boards.php \
  sustainable-catalyst-library/assets/js/sc-library-boards.js \
  sustainable-catalyst-library/assets/css/sc-library-boards.css \
  sustainable-catalyst-library/templates/library-board-studio.php \
  sustainable-catalyst-library-v1.4.0.zip; do
  if [[ ! -f "$required" ]]; then
...
if grep -RInE --exclude-dir=.git --exclude='push_library_v1_4_0_to_github.sh' --exclude='install_and_push_library_v1_4_0.sh' \
...
if git diff --cached --quiet; then
  ...
  git commit -m "Build Library v1.4.0 — Whiteboards and Chalkboards"
fi

git push -u origin main

echo
echo "Sustainable Catalyst Library v1.4.0 pushed successfully."
