...
RELEASE_VERSION="4.3.22.1"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Publications Core Runtime Recovery"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v43221-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v43221.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.22.1 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v43221_validation.sh"

grep -q 'Version: 4.3.22.1' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Installed plugin version marker is not v4.3.22.1."
grep -q "public const VERSION = '4.3.22.1'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-publications.php" || fail "Original Publications runtime was not upgraded to v4.3.22.1."
grep -q 'data-sc-field-spotlights="v4.3.22.1"' "$REPO/sustainable-catalyst-library/templates/field-spotlight-single.php" || fail "Standalone Field Spotlight runtime marker mismatch remains."
grep -q "'global-governance' === \$field" "$REPO/sustainable-catalyst-library/includes/class-sc-library-field-spotlights.php" || fail "Canonical Publications route safety net is missing."
grep -q "class-sc-library-citation-studio.php' => 'SC_Library_Citation_Studio'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-extension-bootstrap-v402.php" || fail "v4.3.22 Citation Studio extension was not preserved."
...
printf 'After WordPress plugin deployment, purge page/CDN caches and hard-refresh Publications.\n'
