Energy Systems Intelligence v1.6.0 adds Grid, Storage & Reliability Analysis through Site Intelligence v4.41.0. Provenance-bound spatial profiles and neutral cross-geography comparisons retain source, year, geometry, indicator and unit boundaries and do not infer site suitability, technical potential, grid reliability, outage status, rankings, or recommendations.
...
Sustainable Catalyst Library v5.30.0 adds Evidence Quality & Methodology Intelligence with source-grounded study-design and method reporting profiles, descriptive cross-study methodology comparison, appraisal-readiness signals, and explicit reporting-coverage diagnostics. Reporting completeness is not a quality, truth, causal-validity, or risk-of-bias score; missing structured metadata does not prove a method was absent. Sustainable Catalyst Library v5.29.0 adds Temporal Knowledge & Research Evolution with explicit publication/version/status chronology, historical-availability snapshots, retrospective correction/retraction status, and reproducible change sets. Sustainable Catalyst Library v5.27.0 adds Source Identity, Deduplication & Entity Resolution with exact normalized DOI/persistent-identifier, content-hash, canonical-URL, and source-scoped identity clustering; review-required title/author/year duplicate candidates; and identifier-grounded ORCID, ROR, and dataset DOI/URL resolution. Records are never silently merged or deleted, title/name-only matches never establish cross-source identity, and source identity does not imply evidence truth. Sustainable Catalyst Library v5.26.0 adds Multimodal Scientific Document Intelligence for source-grounded figures, charts, tables, equations, captions, appendices, supplements, and datasets, and integrates those documentary objects into Research Graph Query & Evidence Pathfinding. Sustainable Catalyst Library v5.25.0 adds deterministic Research Graph Query & Evidence Pathfinding with typed traversal across publications, topics, reviewed findings/claims, explicit citations, and explicitly authored hypothesis metadata. Source-grounded relationships are the default; measured similarity/co-occurrence paths are opt-in and remain clearly analytical. Graph paths do not imply truth, causality, consensus, or agreement, and Platform Core remains the durable authority for governed research objects and synthesis. Sustainable Catalyst Library v5.24.0.1 repairs release certification/readiness alignment so current runtime identity, canonical routing, component compatibility, production soak, and cron advisory state are evaluated against the installed release rather than frozen historical milestone versions. Sustainable Catalyst Library v5.24.0 adds cross-publication evidence synthesis, support/contradiction argument paths, and explicitly authored competing-hypothesis visualization while preserving the v5.23 evidence-weighted research overlays. Consensus, truth, hypotheses, and competition are not inferred; Platform Core remains the durable synthesis authority. Sustainable Catalyst Library v5.23.0.1 repairs local validation so pytest is optional on deployment workstations while preserving v5.23.0 behavior. Sustainable Catalyst Library v5.23.0 adds evidence-weighted accepted finding/claim overlays and explicit reviewed contradiction/support relations. Sustainable Catalyst Library v5.22.0 adds visual evidence traceability and source drilldown from selected publications, topics, regions, and relationships while preserving reproducible visual research sessions and all v5.20.0.2 renderer repairs. Sustainable Catalyst Library v5.21.0 adds reproducible visual research sessions, local save/restore, exportable provenance packages, and explicit Workspace handoff packages while preserving all v5.20.0.2 renderer repairs. Sustainable Catalyst Library v5.20.0.2 repairs scientific renderer visibility and recovers peak-preserving 4D terrain topography while preserving asynchronous Publication Library corpus loading, linked scientific views, and visual query. Sustainable Catalyst Library v5.20.0.1 adds asynchronous Publication Library corpus loading and POST-based manifest transport while preserving the linked scientific views and 4D terrain introduced in v5.20.0. Sustainable Catalyst Library v5.19.0 adds a scientific 4D knowledge terrain with selectable analytical elevation, temporal playback, evolving topic regions, publication anchors, and cross-publication linked views while preserving canonical Publication Library scoping. v5.17.1.1 scopes `[sc_library_knowledge_landscape]` to the canonical Publications interface manifest rather than every public WordPress record. Corpus mode analyzes the publications actually surfaced by the Publication Library; unrelated pages, support content, Foundation documents, and other indexed WordPress object types are excluded. Direct backend corpus calls without a manifest safely fall back to public `post` records only.
...
Energy Systems Intelligence v1.0.0 is the Integrated Sustainable Energy Systems Platform certification release. It preserves the complete v0.1.0-v0.9.0 stack and adds nine-layer release lineage, six governed cross-product contracts, a portable integrated-study contract, and a repository/domain-contract structural certification report. Library remains the active host; Research Librarian, Lab, Workbench, Site Intelligence, and Decision Studio are contract-ready handoff targets only. Structural certification does not claim scientific validation, site suitability, financial advice, live deployment state, automated ranking, winner selection, or policy recommendation.

Energy Systems Intelligence v0.9.0 adds Energy Decision Intelligence while preserving Global Energy Intelligence v0.8.0 and the full v0.1.0-v0.8.0 stack. Twelve governed criteria across nine dimensions feed a portable decision-packet contract, neutral comparison matrix, and readiness inspector. Readiness checks completeness, provenance, uncertainty, unit compatibility, and period compatibility; it is not a merit score. The system does not normalize unlike quantities, assign hidden weights, create a composite sustainability score, rank alternatives, select a winner, or make investment/policy recommendations.
...
= 5.35.0 =
* Adds Rust Evidence Graph Acceleration & Native Query Engine.
* Native runtime v0.2.0 adds filtered neighborhoods, reachability, connected components, induced subgraphs, and structural graph statistics.
* Python retains relationship policy, provenance, evidence semantics, analytical-edge opt-in, and Platform Core governance boundaries.

= 5.34.0 =
* Adds Living Evidence & Research Evolution with immutable review snapshot comparison, explicit change tracking, human-reviewed update candidates, and descriptive evolution chronology.
= 5.29.0 =
...
* Added deterministic source clusters from exact DOI/persistent identifier, content hash, normalized canonical URL, and source-scoped persistent identity.
* Added same-work/version-family diagnostics without automatic replacement or deletion of source records.
* Added review-required bibliographic duplicate candidates from exact normalized title + first author + publication year; candidate status does not establish identity.
* Added ORCID author, ROR institution, and dataset DOI/normalized-URL identity resolution.
* Added Source Identity nodes and relationships to Research Graph Query and the Knowledge Landscape.
* Preserves v5.26 scientific-document intelligence, v5.25 graph/pathfinding, and v5.24 evidence synthesis.
* Platform Core remains the durable authority for governed research objects and synthesis.

= 5.26.0 =
* Added Multimodal Scientific Document Intelligence with backend 2.37.0.
* Added source-grounded figure, chart, table, equation, caption, appendix, supplement, and dataset objects.
* Added exact in-text scientific cross-reference spans and graph relationships.
* Added Scientific Objects view and research-graph/pathfinding integration.
* No chart values are inferred from pixels; OCR text is not treated as verified without explicit verification metadata.
* Platform Core remains the durable authority for governed research objects.

= 5.25.0 =
* Adds deterministic Research Graph Query over the canonical Publication Library corpus.
* Adds Evidence Pathfinder with bounded, direction-aware graph traversal.
* Defaults paths to explicit citations, metadata/reviewed concept associations, reviewed evidence bindings, reviewed support/contradiction, and explicit hypothesis membership.
* Keeps embedding similarity and measured co-occurrence out of evidence paths unless the researcher explicitly opts in.
* Adds WordPress REST proxies and Research Library UI controls for graph query, evidence-path discovery, and path highlighting.
* Preserves v5.24.0 synthesis rules: no inferred consensus, hypothesis, competition, truth, causality, or automatic claim creation.
* Platform Core remains the durable authority for governed research objects and cross-study synthesis.
* Backend version: 2.36.0.

= 5.16.0 =
* Adds Publication Visualization Foundations backed by Python backend v2.27.0.
* Adds reviewed Citation Network, Concept Map, Finding Map, and Claim Map specifications.
* Adds `[sc_library_publication_visualizations]` for accessible Research Library visualization delivery.
* Adds explicit Platform Core cross-product visual research handoff; no automatic truth, finding, claim, or citation inference.


= 5.15.0 =
* Adds source-anchored entity, finding, and claim extraction candidates in Python backend v2.26.0.
* Adds deterministic metadata/rule-based extraction with explicit confidence, source locator, chunk ordinal, and character spans.
* Adds human review states before any finding/claim can enter Platform Core.
* Adds allowlisted Platform Core finding/claim promotion operations tied to an explicit Core research project; Core does not generate or judge the candidate.
* Supersedes pending/accepted extraction candidates automatically when their source record content hash changes.
* Preserves v5.14 citation graph/lineage, v5.13 hybrid retrieval, and v5.12 Platform Core bridge behavior.

= 5.14.0 =
* Adds a declared/imported citation graph with exact identifier resolution and unresolved-reference preservation.
* Adds bounded citation neighborhoods enriched with durable Platform Core bindings.
* Adds explicit governed scholarly-citation promotion into Platform Core; no automatic truth or claim promotion.
* Adds /v1/citations/readiness, citation list/graph, metadata-import, and Core-handoff backend contracts in backend v2.25.0.
...
* Adds Core capability discovery, durable Library↔Core bindings, an idempotent governed sync outbox, and binding reconciliation.
* Keeps raw chunks in Library and disables automatic truth/claim promotion.
* Surfaces Core bridge readiness in the Python Backend admin screen without exposing the Core write credential to WordPress/browser code.
...
* Preserves the 75-concept/63-relationship energy foundation, source-bound numeric registry, all 30 EISD indicator definitions/contracts, seven renewable technology objects, and six resource classes.
* Adds a governed four-model Energy Balance & Systems framework with three executable explicit-input calculations and one portable scenario contract.
* Adds deterministic conversion-chain calculations with stage input, efficiency, output, loss, final output, total loss, and overall efficiency.
* Adds supply-demand accounting across domestic supply, imports, storage discharge, final demand, exports, storage charge, declared losses, tolerance, and residual.
* Adds capacity-factor generation arithmetic from explicit capacity, capacity factor, and hours without inferring technology-specific performance.
* Adds an Energy Balance interface, GET-only backend routes, WordPress REST facades, JSON Schemas, and a machine-readable balance-model export.
* Activates computational handoff contracts for later Lab and Workbench integration without modifying those separate products.
* Does not perform time-series dispatch, storage-physics simulation, grid reliability/adequacy analysis, economic optimization, scenario persistence, automatic ranking, or policy recommendation.

= Energy Systems Intelligence 0.4.0 / Backend 2.10.0 =

* Preserves the 75-concept/63-relationship energy foundation, v0.2.0 numeric registry, and all 30 v0.3.0 EISD indicator definitions/contracts.
* Adds seven governed renewable technology objects: solar photovoltaic, solar thermal, wind, hydropower, tidal, wave, and bioenergy.
* Adds six normalized resource classes: solar, wind, hydrological, tidal, wave, and biomass feedstock resources.
* Adds technology assessment contracts carrying geography, period, configuration, resource evidence, efficiency/capacity/output placeholders, cost, lifecycle emissions, land/water, reliability, constraints, system context, methodology, sources, assumptions, and uncertainty.
* Adds resource observation contracts requiring metric, value, unit, spatial/temporal resolution, method/model, source vintage, uncertainty, constraints, and provenance; biomass additionally requires feedstock origin, competing-use, and land-use context.
* Adds GET-only technology/resource backend routes, matching WordPress REST facades, JSON Schemas, and a machine-readable renewable technology/resource export.
* Activates contract-ready handoffs to Lab and Site Intelligence without modifying those separate products.
* Does not load current resource-potential datasets, universal performance profiles, maturity scores, site suitability, rankings, scenario execution, or automatic policy recommendations.
...
== 5.29.0 ==
* Adds explicit temporal event chronology, historical-availability snapshots, retrospective status, and reproducible research-state change sets.
* Later corrections/retractions are not silently projected backward into historical snapshots.
* Adds [sc_library_temporal_evolution] and backend v2.40.0.

== 5.30.0 ==
* Adds source-grounded Evidence Quality & Methodology Intelligence and backend v2.41.0.
* Structures explicit study design, population, sample, geography, time period, interventions/exposures, comparators, outcomes, variables, analytical methods, uncertainty, limitations, funding/conflicts, registration, data/code/materials, replication, randomization, masking, and ethics metadata.
