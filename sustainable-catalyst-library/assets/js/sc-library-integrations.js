(() => {
  ...
  const saveWorkspace = (data) => {
    data.schema = workspaceSchema;
    data.version = shared.version || data.version || '1.13.0';
    data.updatedAt = now();
  ...
  const contextItems = (ws) => [
    ...
    ...ws.books.map((x) => ({type:'book', id:x.id, title:x.title, object:x})),
  ];
  ...
  const buildPackage = ({targetId, entry, purpose, question}, ws) => ({
    ...
    target: targetId,
    created_at: now(),
    source: {application:'sustainable-catalyst-library',version:shared.version || '1.13.0',url:window.location.href},
