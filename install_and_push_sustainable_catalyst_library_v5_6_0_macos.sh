...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Dynamic Library Explorer & Progressive Discovery"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v560-validation-venv"
...
[[ -f "$ZIP" ]] || fail "v5.6.0 repository ZIP was not found."
REPO="$(find_repo || true)"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v560.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Installing v5.6.0 repository payload"
rsync -a --exclude='.git/' --exclude='.DS_Store' --exclude='__MACOSX/' --exclude='.venv/' --exclude='.pytest_cache/' --exclude='__pycache__/' "$SOURCE/" "$REPO/"

say "Running v5.6.0 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v560_validation.sh"

grep -Fq 'Version: 5.6.0' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin version marker is not v5.6.0."
grep -Fq "SC_LIBRARY_VERSION', '5.6.0'" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "SC_LIBRARY_VERSION is not v5.6.0."
grep -Fq "public const VERSION = '5.6.0'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-python-backend.php" || fail "Python backend bridge is not v5.6.0."
...
grep -Fq '/v1/explorer/bootstrap' "$REPO/library-backend/app/main.py" || fail "Explorer bootstrap API contract is missing."
grep -Fq 'class SC_Library_Dynamic_Explorer' "$REPO/sustainable-catalyst-library/includes/class-sc-library-dynamic-explorer.php" || fail "Dynamic Explorer bridge is missing."
grep -Fq 'sc-library-dynamic-explorer-v560.js' "$REPO/sustainable-catalyst-library/includes/class-sc-library-dynamic-explorer.php" || fail "Dynamic Explorer client asset contract is missing."
