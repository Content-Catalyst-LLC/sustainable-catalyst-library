#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="4.3.40"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — 4.3 Branch Production Hardening"
...
ensure_pytest(){ local c base_py venv; say "Preparing isolated validation environment"; for c in "${SC_LIBRARY_VALIDATION_PYTHON:-}" "$DOWNLOADS/.sc-library-v4340-validation-venv/bin/python3" "$DOWNLOADS/.sc-library-v4339-validation-venv/bin/python3" "$DOWNLOADS/.sc-library-v4338-validation-venv/bin/python3" "$(command -v python3 2>/dev/null || true)"; do [[ -n "$c" && -x "$c" ]] || continue; if "$c" -m pytest --version >/dev/null 2>&1; then export SC_LIBRARY_VALIDATION_PYTHON="$c"; printf 'pytest ready: %s\n' "$c"; return; fi; done; base_py="$(command -v python3 2>/dev/null || true)"; [[ -n "$base_py" ]] || fail "Python 3 is required."; venv="$DOWNLOADS/.sc-library-v4340-validation-venv"; rm -rf "$venv"; "$base_py" -m venv "$venv"; "$venv/bin/python3" -m pip install -q --upgrade pip; "$venv/bin/python3" -m pip install -q 'pytest>=8'; export SC_LIBRARY_VALIDATION_PYTHON="$venv/bin/python3"; }
ZIP="$(find_release_zip)"; [[ -f "$ZIP" ]] || fail "v4.3.40 repository ZIP was not found."
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4340.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT; unzip -q "$ZIP" -d "$TMP"; MAIN_FILE="$(find "$TMP" -type f -path '*/sustainable-catalyst-library/sustainable-catalyst-library.php' -print | head -1)"; [[ -f "$MAIN_FILE" ]] || fail "Could not locate plugin entry point."; SOURCE="$(dirname "$(dirname "$MAIN_FILE")")"
...
say "Running v4.3.40 validation"; SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v4340_validation.sh"
MAIN="$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php"; HARD="$REPO/sustainable-catalyst-library/includes/class-sc-library-hardening.php"; ROUTE="$REPO/sustainable-catalyst-library/includes/class-sc-library-canonical-route-identity.php"; BOOT="$REPO/sustainable-catalyst-library/includes/class-sc-library-extension-bootstrap-v402.php"; PAGE="$REPO/RESEARCH_LIBRARY_PAGE_v4.3.40.html"; README="$REPO/sustainable-catalyst-library/readme.txt"; PORT="$REPO/sustainable-catalyst-library/includes/class-sc-library-research-portability-preservation.php"
say "Checking release boundaries"
grep -Fq 'Version: 4.3.40' "$MAIN" || fail "Plugin version marker is not v4.3.40."
grep -Fq "SC_LIBRARY_VERSION', '4.3.40'" "$MAIN" || fail "SC_LIBRARY_VERSION is not v4.3.40."
grep -Fq "BRANCH_VERSION = '4.3.40'" "$HARD" || fail "Production hardening branch version is not v4.3.40."
grep -Fq "BRANCH_SCHEMA = 'sc-library-v43-production-certification/1.0'" "$HARD" || fail "Branch certification schema is missing."
grep -Fq "'/runtime/production-readiness'" "$HARD" || fail "Public production-readiness route is missing."
grep -Fq "'/runtime/production-readiness/details'" "$HARD" || fail "Admin production-readiness detail route is missing."
grep -Fq "'branch_release_gate'" "$HARD" || fail "Distinct branch release gate is missing."
grep -Fq "'first_party_only' => true" "$HARD" || fail "First-party-only release contract is missing."
grep -Fq "'network_calls_performed' => false" "$HARD" || fail "Network-call truth boundary is missing."
grep -Fq "'upstream_health_release_blocking' => false" "$HARD" || fail "Upstream non-blocking boundary is missing."
grep -Fq "'private_record_content_inspected' => false" "$HARD" || fail "Private-content diagnostic boundary is missing."
grep -Fq "'__return_true' === \$endpoint['permission_callback']" "$HARD" || fail "Private REST permission certification is missing."
grep -Fq "public const VERSION = '4.3.40'" "$ROUTE" || fail "Canonical identity runtime is not v4.3.40."
grep -Fq 'data-sc-library-account-continuity="v4.3.40"' "$ROUTE" || fail "Account-continuity render marker is not v4.3.40."
grep -Fq 'MODULE_COUNT = 44' "$BOOT" || fail "Extension bootstrap count changed unexpectedly."
grep -Fq '[sc_library_readiness_status show_categories="true"]' "$PAGE" || fail "Research Infrastructure is missing Production Status."
grep -Fq '[sc_research_portability title="Research Portability & Preservation"]' "$PAGE" || fail "v4.3.39 Research Portability was not preserved."
grep -Fq "VERSION = '4.3.39'" "$PORT" || fail "v4.3.39 portability implementation was unexpectedly rewritten."
grep -Fq 'Stable tag: 4.3.40' "$README" || fail "Readme stable tag is not v4.3.40."
say "Checking v4.3.39 -> v4.3.40 upgrade delta"; DELTA_COUNT="$(git -C "$REPO" status --porcelain | wc -l | tr -d ' ')"; [[ "$DELTA_COUNT" == "16" ]] || fail "Expected a 16-file v4.3.40 delta; found $DELTA_COUNT files."; [[ -z "$(git -C "$REPO" status --porcelain | awk '$1 ~ /^D/ {print}')" ]] || fail "v4.3.40 unexpectedly deletes tracked files."
