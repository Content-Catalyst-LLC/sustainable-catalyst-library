#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="4.3.9"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Field Spotlight Interaction Parity, White Active State & Sharp-Edge Refinement"
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v439-validation-venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v439.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.9 release validation"
bash "$REPO/tests/run_v439_validation.sh"
...
printf 'Public Field Spotlights: true additional-fields accordion, white currently-playing panel, sharp square geometry, Homepage Spotlight-scale Article Map lead, thumbnail editorial rows, and preserved eight-panel autoplay behavior.\n'
