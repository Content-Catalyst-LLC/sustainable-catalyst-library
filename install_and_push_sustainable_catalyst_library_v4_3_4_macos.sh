#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="4.3.4"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Field Spotlight Data Architecture & Series Panel Registry"
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v434-validation-venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v434.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.4 release validation"
bash "$REPO/tests/run_v434_validation.sh"
...
printf 'Field Spotlight contract: 14 fields / 170 flattened Article Map panels / default 8-panel opening tier / 4 supporting slots (2-8 configurable).\n'
printf 'Existing public shortcodes remain: [sc_publications] and [sc_homepage_spotlight]\n'
