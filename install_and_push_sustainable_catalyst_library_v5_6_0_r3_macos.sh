#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="5.6.0-R3"
PLUGIN_VERSION="5.6.0.3"
RELEASE_NAME="Sustainable Catalyst Library v5.6.0 R3 — Visible Research Network, University Search & Local Library Access"
DOWNLOADS="${HOME}/Downloads"
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
REPOSITORY_BASENAME="sustainable-catalyst-library-v5.6.0-R3-repository.zip"
CHECKSUM_BASENAME="SHA256SUMS-v5.6.0-R3.txt"
...
find_release_zip(){
  ...
  find "$DOWNLOADS" -maxdepth 3 -type f -name 'sustainable-catalyst-library-v5.6.0-R3-repository*.zip' -print 2>/dev/null | sort | tail -1
}
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v560r3-validation-venv"
...
[[ -f "$ZIP" ]] || fail "v5.6.0 R3 repository ZIP was not found."
REPO="$(find_repo || true)"
...
BACKUP="$DOWNLOADS/sustainable-catalyst-library-before-v5.6.0-R3-$(date +%Y%m%d-%H%M%S).zip"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v560r3.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Installing v5.6.0 R3 repository payload"
rsync -a --exclude='.git/' --exclude='.DS_Store' --exclude='__MACOSX/' --exclude='.venv/' --exclude='.pytest_cache/' --exclude='__pycache__/' "$SOURCE/" "$REPO/"

say "Running preservation + regression validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v560r3_validation.sh"

grep -Fq 'Version: 5.6.0.3' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin version marker is not 5.6.0.3."
grep -Fq "SC_LIBRARY_VERSION', '5.6.0.3'" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "SC_LIBRARY_VERSION is not 5.6.0.3."
grep -Fq 'class SC_Library_Capability_Hub' "$REPO/sustainable-catalyst-library/includes/class-sc-library-capability-hub.php" || fail "Capability Hub is missing."
grep -Fq '[sc_library_capability_hub' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R3.html" || fail "R3 page does not include the capability hub."
grep -Fq '"protected_shortcodes"' "$REPO/LIBRARY_CAPABILITY_MANIFEST_v5.6.0-R3.json" || fail "Capability preservation manifest is missing."
grep -Fq '[sc_research_network_console title="Research Network Console"]' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R3.html" || fail "Research Network Console is missing."
grep -Fq 'University College Dublin' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R3.html" || fail "University College Dublin is not visibly represented."
grep -Fq '[sc_public_library_network title="Public Library Network & Local Access"]' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R3.html" || fail "Public Library Network is not directly visible."
grep -Fq 'display="expanded"' "$REPO/RESEARCH_LIBRARY_PAGE_v5.6.0-R3.html" || fail "Expanded capability visibility is missing."
grep -Fq 'Find a Library Near You' "$REPO/sustainable-catalyst-library/includes/class-sc-library-public-library-network.php" || fail "Local library discovery is missing."
grep -Fq 'height:min(72vh,760px)' "$REPO/sustainable-catalyst-library/assets/css/sc-library-capability-hub-v560r3.css" || fail "Bounded capability workspace CSS is missing."
grep -Fq '__version__ = "1.1.0"' "$REPO/library-backend/app/__init__.py" || fail "Python backend service is not v1.1.0."

if [[ -z "$(git -C "$REPO" status --porcelain)" ]]; then
  printf 'Repository already matches v5.6.0 R3; nothing to commit.\n'
else
...
printf '\nPASS - Sustainable Catalyst Library v5.6.0 R3 installed, validated, committed, and pushed.\n'
