...
RELEASE_ZIP="${2:-$BUNDLE_DIR/sustainable-catalyst-library-v5.12.0-repository.zip}"
REMOTE_URL="https://github.com/Content-Catalyst-LLC/sustainable-catalyst-library.git"
TAG="v5.12.0"
TMP="$(mktemp -d /tmp/sc-library-v5120.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
if git diff --cached --quiet; then
  ...
  git commit -m "Library v5.12.0 Platform Core Research Bridge"
fi
...
if git rev-parse "$TAG" >/dev/null 2>&1; then
  ...
  git tag -a "$TAG" -m "Sustainable Catalyst Library v5.12.0 — Platform Core Research Bridge"
fi

git push origin HEAD
git push origin "$TAG"

echo "PASS: Knowledge Library v5.12.0 committed, tagged, and pushed."
