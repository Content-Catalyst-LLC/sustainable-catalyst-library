...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Twelve-Topic Two-Tier Homepage Spotlight"
...
find_repo() {
  ...
  for candidate in \
    ...
    "$DOWNLOADS/sustainable-catalyst-library-repository" \
  ...
  while IFS= read -r candidate; do
    ...
    if is_library_repo "$candidate"; then
      ...
      if [[ "$remote" == *"sustainable-catalyst-library"* ]]; then
        printf '%s\n' "$candidate"
        return 0
      fi
    fi
  done < <(find "$DOWNLOADS" "$HOME/Documents" -maxdepth 4 -type d -name .git -print 2>/dev/null | sed 's#/.git$##')

  return 1
}

verify_checksum() {
  local zip="$1" manifest candidate expected actual
  candidate="$(basename "$zip")"
  for manifest in \
    "$SCRIPT_DIR/SHA256SUMS-v${RELEASE_VERSION}.txt" \
    "$SCRIPT_DIR/SHA256SUMS-core-v${RELEASE_VERSION}.txt" \
    "$DOWNLOADS/SHA256SUMS-v${RELEASE_VERSION}.txt" \
    "$DOWNLOADS/SHA256SUMS-core-v${RELEASE_VERSION}.txt"; do
    [[ -f "$manifest" ]] || continue
    expected="$(awk -v file="$candidate" '$2 == file {print $1}' "$manifest" | head -n 1)"
    [[ -n "$expected" ]] || continue
  ...
  printf 'Checksum manifest not found beside the installer or in Downloads; continuing with release-contract validation.\n'
}
...
(
  cd "$(dirname "$REPO")"
  zip -qry "$BACKUP" "$(basename "$REPO")" -x '*/.git/*' '*/.DS_Store' '*/__MACOSX/*' '*/.pytest_cache/*' '*/__pycache__/*' '*.pyc'
)
printf 'Safety backup: %s\n' "$BACKUP"

TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v420.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.2.0 release validation"
grep -q 'Version: 4.2.0' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin header version does not match 4.2.0."
grep -q "SC_LIBRARY_VERSION', '4.2.0" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin constant does not match 4.2.0."
grep -q 'Stable tag: 4.2.0' "$REPO/sustainable-catalyst-library/readme.txt" || fail "WordPress stable tag does not match 4.2.0."
bash "$REPO/tests/run_v420_validation.sh"
...
if [[ -z "$(git -C "$REPO" status --porcelain)" ]]; then
  ...
  git -C "$REPO" add -A
  git -C "$REPO" commit -m "Sustainable Catalyst Library v${RELEASE_VERSION} — Twelve-Topic Two-Tier Homepage Spotlight"
fi
...
printf 'Homepage shortcode: [sc_homepage_spotlight]\n'
printf 'Administration: SC Library → Homepage Spotlight → Add or align the 12-topic library set\n'
printf 'Editorial requirement: curate five valid articles for each new topic before it appears publicly.\n'
