...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Knowledge Library Console Presentation Refresh"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v412.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.1.2 release validation"
grep -q 'Version: 4.1.2' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin header version does not match 4.1.2."
grep -q "SC_LIBRARY_VERSION', '4.1.2" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin constant does not match 4.1.2."
grep -q 'Stable tag: 4.1.2' "$REPO/sustainable-catalyst-library/readme.txt" || fail "WordPress stable tag does not match 4.1.2."
bash "$REPO/tests/run_v412_validation.sh"
...
if [[ -z "$(git -C "$REPO" status --porcelain)" ]]; then
  ...
  git -C "$REPO" add -A
  git -C "$REPO" commit -m "Sustainable Catalyst Library v${RELEASE_VERSION} — Knowledge Library Console Presentation Refresh"
fi
...
printf 'Homepage shortcode: [sc_homepage_spotlight] (automatic 14-second rotation by default)\n'
