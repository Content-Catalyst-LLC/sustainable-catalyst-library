...
def native_graph_runtime_status() -> dict[str, Any]:
    ...
    result: dict[str, Any] = {
        ...
        "binary_path": path,
        "available": False,
        "capabilities": ["bounded-pathfinding-foundation"],
    ...
    try:
        completed = subprocess.run([path, "status"], capture_output=True, text=True, timeout=3, check=True)
...
def native_path_steps(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="sc-library-native-graph-") as tmp:
        ...
        nodes_file.write_text("".join(f"{str(n.get('id') or '').replace(chr(9),' ')}\t{str(n.get('kind') or '').replace(chr(9),' ')}\n" for n in nodes if n.get("id")), encoding="utf-8")
        ...
        for idx, edge in enumerate(edges):
            ...
            basis = str(edge.get("relationship_basis") or edge.get("basis") or edge.get("type") or "relationship").replace("\t", " ")
            edge_lines.append(f"{idx}\t{source}\t{target}\t{basis}\t{1 if edge.get('directed') else 0}\n")
        edges_file.write_text("".join(edge_lines), encoding="utf-8")
        cmd = [
            path, "pathfind",
            "--nodes", str(nodes_file),
            "--edges", str(edges_file),
            "--starts", ",".join(start_ids),
            "--targets", ",".join(sorted(target_ids)),
            "--target-kinds", ",".join(sorted(target_kinds)),
        ...
        for line in lines[1:]:
            ...
            for token in [x for x in parts[3].split(",") if x]:
                ...
                if from_id != current:
                    steps = []
                    break
                basis = str(edge.get("relationship_basis") or edge.get("basis") or edge.get("type") or "relationship")
                steps.append({
                    "from_id": from_id,
                    "to_id": to_id,
            ...
            if not steps or current != target:
                continue
            out.append({"source_node_id": source, "target_node_id": target, "node_ids": node_ids, "steps": steps})
        return out
