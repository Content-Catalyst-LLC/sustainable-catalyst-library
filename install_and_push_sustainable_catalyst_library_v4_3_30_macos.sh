...
RELEASE_VERSION="4.3.30"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Unified Research Projects & Source Bundles"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v4330-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
[[ -f "$ZIP" ]] || fail "v4.3.30 repository ZIP was not found."
REPO="$(find_repo || true)"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4330.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.30 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v4330_validation.sh"
...
PAGE="$REPO/RESEARCH_LIBRARY_PAGE_v4.3.30.html"

say "Checking release boundaries"
grep -q 'Version: 4.3.30' "$MAIN" || fail "Plugin version marker is not v4.3.30."
grep -q "SC_LIBRARY_VERSION', '4.3.30'" "$MAIN" || fail "SC_LIBRARY_VERSION is not v4.3.30."
...
grep -q 'MODULE_COUNT = 35' "$BOOT" || fail "Extension module count is not 35."
grep -q 'class-sc-library-unified-research-projects-source-bundles.php' "$BOOT" || fail "v4.3.30 project module is not registered."
grep -q "VERSION = '4.3.30'" "$PROJECTS" || fail "Unified Research Projects module version is not v4.3.30."
...
grep -q "public const VERSION = '4.3.30'" "$ROUTE" || fail "Route/identity health runtime is not version-aligned."
...
[[ -f "$PAGE" ]] || fail "Research Library v4.3.30 page artifact is missing."
...
say "Complete"
printf 'Installed Sustainable Catalyst Library v%s — Unified Research Projects & Source Bundles\n' "$RELEASE_VERSION"
printf 'WordPress plugin ZIP: %s\n' "$SCRIPT_DIR/$PLUGIN_BASENAME"
printf 'Research Library page replacement: %s\n' "$SCRIPT_DIR/RESEARCH_LIBRARY_PAGE_v4.3.30.html"
