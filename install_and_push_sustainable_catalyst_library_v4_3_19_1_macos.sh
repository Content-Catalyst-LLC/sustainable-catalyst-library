...
RELEASE_VERSION="4.3.19.1"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Access-First Layout & Editorial Compression"
...
verify_checksum(){
  ...
  for manifest in \
    ...
    "$DOWNLOADS/SHA256SUMS-v${RELEASE_VERSION}.txt"; do
    [[ -f "$manifest" ]] || continue
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v43191-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v43191.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.19.1 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v43191_validation.sh"
...
printf 'Research Library page replacement: RESEARCH_LIBRARY_PAGE_v4.3.19.1.html\n'
printf 'After plugin deployment, replace the Research Library page body with the v4.3.19.1 page artifact and clear caches.\n'
printf 'The Publications page does not require replacement.\n'
