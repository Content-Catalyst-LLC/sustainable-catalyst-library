...
RELEASE_VERSION="4.3.27"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Canonical Routing, Identity & Account Continuity"
...
ensure_pytest(){
  ...
  for candidate in \
    ...
    "$DOWNLOADS/.sc-library-v4325-validation-venv/bin/python3" \
  ...
  venv="$DOWNLOADS/.sc-library-v4327-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
[[ -f "$ZIP" ]] || fail "v4.3.27 repository ZIP was not found."
REPO="$(find_repo || true)"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4327.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.27 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v4327_validation.sh"
...
PAGE="$REPO/RESEARCH_LIBRARY_PAGE_v4.3.27.html"

say "Checking release boundaries"
grep -q 'Version: 4.3.27' "$MAIN" || fail "Plugin version marker is not v4.3.27."
grep -q "SC_LIBRARY_VERSION', '4.3.27'" "$MAIN" || fail "SC_LIBRARY_VERSION is not v4.3.27."
...
grep -q 'MODULE_COUNT = 32' "$BOOT" || fail "Extension module count is not 32."
grep -q 'class-sc-library-canonical-route-identity.php' "$BOOT" || fail "Canonical route/identity module is not registered."
...
grep -q "HEALTH_ROUTE = '/runtime/identity-health'" "$ROUTE" || fail "Route/identity health endpoint is missing."
grep -q 'wp_safe_redirect( $target, 301' "$ROUTE" || fail "Legacy public route is not configured for 301 canonical redirect."
grep -q "'api_namespace_preserved' => true" "$ROUTE" || fail "API namespace preservation contract is missing."
grep -q 'sc_library_my_libraries_v4319' "$ROUTE" || fail "My Libraries continuity contract is missing."
grep -q 'sc_library_source_collections_v4322' "$ROUTE" || fail "Citation Studio collection continuity contract is missing."
grep -q 'sc_library_research_documents_v4323' "$ROUTE" || fail "Research Document Builder continuity contract is missing."
grep -q 'sc_library_course_plan_v4321' "$ROUTE" || fail "Open Course plan continuity contract is missing."
...
[[ -f "$PAGE" ]] || fail "Research Library v4.3.27 page artifact is missing."
grep -q '\[sc_library_account_continuity\]' "$PAGE" || fail "Research Library page does not expose account continuity."
...
say "Complete"
printf 'Installed Sustainable Catalyst Library v%s — Canonical Routing, Identity & Account Continuity\n' "$RELEASE_VERSION"
printf 'WordPress plugin ZIP: %s\n' "$SCRIPT_DIR/$PLUGIN_BASENAME"
printf 'Research Library page replacement: %s\n' "$SCRIPT_DIR/RESEARCH_LIBRARY_PAGE_v4.3.27.html"
printf 'Canonical public route: https://sustainablecatalyst.com/knowledge-libraries/\n'
printf 'Post-deploy health check: https://sustainablecatalyst.com/wp-json/sc-library/v1/runtime/identity-health\n'
