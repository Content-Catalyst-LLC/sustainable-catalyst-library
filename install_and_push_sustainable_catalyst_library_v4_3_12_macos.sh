#!/usr/bin/env bash
set -euo pipefail
RELEASE_VERSION="4.3.12"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Dedicated Field Spotlight Panel-Content Persistence"
...
find_release_zip(){
  local candidate
  for candidate in "$SCRIPT_DIR/sustainable-catalyst-library-v${RELEASE_VERSION}-repository.zip" "$DOWNLOADS/sustainable-catalyst-library-v${RELEASE_VERSION}-repository.zip"; do
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v4312-validation-venv"
...
ZIP="$(find_release_zip)"; [[ -f "$ZIP" ]] || fail "Release repository ZIP was not found."
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4312.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
rsync -a --delete --exclude='.git/' --exclude='.DS_Store' --exclude='__MACOSX/' --exclude='.venv/' "$SOURCE/" "$REPO/"
say "Running v4.3.12 release validation"
bash "$REPO/tests/run_v4312_validation.sh"
...
say "Complete"
printf 'Installed %s\n' "$RELEASE_NAME"
printf 'Administration: SC Library -> Field Spotlights\n'
printf 'Console: readiness dashboard, searchable/filterable panel inventory, thumbnail rows, and Library source search inside supporting slots.\n'
printf 'Field Spotlight content saves now use a dedicated panel-content store: hero copy and supporting article selections are isolated from structural field settings, read-after-write verified, counted in the save result, and immediately clear public/model caches. v4.3.9 presentation behavior is preserved.\n'
printf 'Public shortcodes remain: [sc_field_spotlights], [sc_field_spotlight field="global-governance"], [sc_publications], and [sc_homepage_spotlight].\n'
