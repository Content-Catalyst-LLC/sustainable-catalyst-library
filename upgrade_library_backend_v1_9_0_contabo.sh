...
ZIP="${1:-/tmp/sustainable-catalyst-library-backend-v1.9.0.zip}"
[[ -f "$ZIP" ]] || { echo "ERROR: backend ZIP not found: $ZIP" >&2; exit 1; }
[[ -f "$LIVE/.env" ]] || { echo "ERROR: existing backend .env not found at $LIVE/.env" >&2; exit 1; }
ts="$(date +%Y%m%d-%H%M%S)"
tmp="$(mktemp -d /tmp/sc-library-backend-v190.XXXXXX)"
trap 'rm -rf "$tmp"' EXIT
env_backup="/tmp/sc-library-backend-v1.9.0.env.$ts"
app_backup="$BASE/library-backend.before-v1.9.0-$ts"
...
[[ "$version" == "1.9.0" ]] || { echo "ERROR: expected backend v1.9.0, got: $version" >&2; exit 1; }
python3 -c 'import json,sys; d=json.load(sys.stdin); r=d.get("reliability_policy",{}); g=d.get("framework",{}).get("governance",{}); assert d.get("schema")=="sc-biomedical-evidence-graph-manifest/1.1"; assert r.get("title_only_merge") is False; assert r.get("deterministic_ordering") is True; assert r.get("partial_source_failures_are_contained") is True; assert g.get("semantic_equivalence_asserted") is False; assert g.get("causal_relationship_inferred") is False; assert g.get("pooled_effect_generated") is False; assert g.get("clinical_recommendation_generated") is False' <<<"$graph_json"
printf '\nPASS - Library backend v1.9.0 upgraded; biomedical graph reliability and provenance contract is active.\n'
