...
if ! grep -q "Workbench, Decision Studio, and Site Intelligence" sustainable-catalyst-library/readme.txt; then
  echo "ERROR: Integration release marker is missing."
  exit 1
fi
for required in \
  sustainable-catalyst-library/includes/class-sc-library-integrations.php \
  sustainable-catalyst-library/assets/js/sc-library-integrations.js \
  sustainable-catalyst-library/assets/css/sc-library-integrations.css \
  sustainable-catalyst-library/templates/library-integrations.php \
  sustainable-catalyst-library-v1.5.0.zip; do
  if [[ ! -f "$required" ]]; then
...
if grep -RInE --exclude-dir=.git --exclude='push_library_v1_5_0_to_github.sh' --exclude='install_and_push_library_v1_5_0.sh' \
...
if git diff --cached --quiet; then
  ...
  git commit -m "Build Library v1.5.0 — Workbench, Decision Studio, and Site Intelligence integration"
fi

git push -u origin main

echo
echo "Sustainable Catalyst Library v1.5.0 pushed successfully."
