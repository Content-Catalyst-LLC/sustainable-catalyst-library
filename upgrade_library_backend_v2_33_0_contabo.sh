#!/usr/bin/env bash
set -Eeuo pipefail
ARCHIVE="${1:-/tmp/sustainable-catalyst-library-backend-v2.33.0.zip}"
ROOT=/opt/sustainable-catalyst/library-backend
BACKUP_ROOT=/opt/sustainable-catalyst/backups
TMP="$(mktemp -d /tmp/sc-library-v2330.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
SRC="$TMP/sustainable-catalyst-library-backend-v2.33.0"
[[ -f "$SRC/app/__init__.py" ]] || fail "backend payload missing"
grep -q '__version__ = "2.33.0"' "$SRC/app/__init__.py" || fail "payload is not backend v2.33.0"
...
if [[ -d "$ROOT" ]]; then tar -C "$(dirname "$ROOT")" -czf "$BACKUP_ROOT/library-backend-before-v2.33.0-$stamp.tgz" "$(basename "$ROOT")"; fi
...
python3 -c 'import json,sys; d=json.load(sys.stdin); print(json.dumps({"ok":d.get("ok"),"version":d.get("version"),"database":d.get("database")},indent=2)); assert d.get("ok") is True; assert d.get("version")=="2.33.0"; c=d.get("capabilities",{}); assert c.get("publication_linked_scientific_views") is True; assert c.get("publication_visual_query_contract") is True; assert c.get("publication_async_corpus_transport") is True; assert c.get("publication_corpus_post_transport") is True; assert c.get("publication_renderer_visibility_repair") is True; assert c.get("publication_4d_terrain_recovery") is True; assert c.get("publication_peak_preserving_terrain_surface") is True; assert c.get("publication_reproducible_visual_sessions") is True; assert c.get("publication_workspace_visual_handoff_package") is True; assert c.get("publication_visual_evidence_trace") is True; assert c.get("publication_source_drilldown") is True' < "$TMP/health.json"
...
echo "PASS: Library backend v2.33.0 Visual Evidence Traceability & Source Drilldown deployed."
