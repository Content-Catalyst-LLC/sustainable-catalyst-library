...
ZIP="${1:-/tmp/sustainable-catalyst-library-backend-v1.0.2.zip}"
[[ -f "$ZIP" ]] || { echo "ERROR: backend ZIP not found: $ZIP" >&2; exit 1; }
[[ -f "$LIVE/.env" ]] || { echo "ERROR: existing backend .env not found at $LIVE/.env" >&2; exit 1; }

ts="$(date +%Y%m%d-%H%M%S)"
tmp="$(mktemp -d /tmp/sc-library-backend-v102.XXXXXX)"
trap 'rm -rf "$tmp"' EXIT

env_backup="/tmp/sc-library-backend-v1.0.2.env.$ts"
app_backup="$BASE/library-backend.before-v1.0.2-$ts"
...
[[ "$version" == "1.0.2" ]] || { echo "ERROR: expected backend v1.0.2, got: $version" >&2; exit 1; }

printf '\nPASS - Sustainable Catalyst Library backend v1.0.2 upgraded and healthy.\n'
