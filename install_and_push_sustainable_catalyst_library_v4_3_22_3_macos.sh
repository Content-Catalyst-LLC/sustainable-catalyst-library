...
RELEASE_VERSION="4.3.22.3"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Publications Server-Authoritative Navigation Recovery"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v432223-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v432223.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.22.3 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v432223_validation.sh"

grep -q 'Version: 4.3.22.3' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Installed plugin version marker is not v4.3.22.3."
grep -q "public const VERSION = '4.3.22.3'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-publications.php" || fail "Core Publications runtime was not upgraded to v4.3.22.3."
grep -q 'data-sc-publications="v4.3.22.3"' "$REPO/sustainable-catalyst-library/templates/publications.php" || fail "Core Publications template runtime mismatch remains."
grep -q 'data-sc-publications-navigation="server"' "$REPO/sustainable-catalyst-library/templates/publications.php" || fail "Core Publications is not marked server-authoritative."
grep -q 'data-sc-field-spotlights="v4.3.22.3"' "$REPO/sustainable-catalyst-library/templates/field-spotlights.php" || fail "Field Spotlight master runtime mismatch remains."
grep -q 'data-sc-field-navigation="server"' "$REPO/sustainable-catalyst-library/templates/field-spotlights.php" || fail "Field Spotlight field navigation is not marked server-authoritative."
grep -q 'major-field tabs are server-authoritative anchors' "$REPO/sustainable-catalyst-library/assets/js/sc-library-publications.js" || fail "Core server-authoritative field contract is missing."
if grep -q "link.addEventListener('click'" "$REPO/sustainable-catalyst-library/assets/js/sc-library-publications.js"; then fail "Core Publications still intercepts a direct field/area link."; fi
grep -q 'do not install a click handler' "$REPO/sustainable-catalyst-library/assets/js/sc-library-field-spotlights.js" || fail "Field Spotlight server-authoritative field contract is missing."
grep -q "url.searchParams.set('sc_publication_field', key)" "$REPO/sustainable-catalyst-library/assets/js/sc-library-field-spotlights.js" || fail "Field Spotlight mobile selector does not navigate through the server field route."
grep -q "public const META_OWNER = '_sc_source_personal_owner'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-citation-studio.php" || fail "v4.3.22 Citation Studio was not preserved."
...
printf 'Neither the Research Library page nor the Publications page body needs replacement for this patch.\n'
printf 'After WordPress plugin deployment, purge WordPress/page/CDN caches and hard-refresh Publications.\n'
