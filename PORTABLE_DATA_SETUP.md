# Library v1.20.0 Portable Data Setup

Library v1.20.0 preserves every earlier export scope and advances the portable schema to `sc-library-portable-export/2.1`. WordPress remains the canonical publishing system; the export is a normalized, application-oriented representation suitable for PostgreSQL, CSV, JSONL, JSON, preservation review, and disaster-recovery testing.
...
1. Upload `sustainable-catalyst-library-v1.20.0.zip` and replace the current plugin.
...
The `readiness_runs` entity preserves aggregate status, score, timestamp, and the generated remediation report. It does not contain API secrets or private workspace content.
