...
RELEASE_VERSION="4.3.20"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Open Course Finder Foundation"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v4320-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4320.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.20 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v4320_validation.sh"
...
printf 'Research Library page replacement: RESEARCH_LIBRARY_PAGE_v4.3.20.html\n'
printf 'After plugin deployment, replace the Research Library page body with the v4.3.20 page artifact and clear caches.\n'
