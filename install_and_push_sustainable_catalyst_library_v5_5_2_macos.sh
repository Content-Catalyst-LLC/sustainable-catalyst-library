...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Backend Operations & Recovery"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v552-validation-venv"
...
[[ -f "$ZIP" ]] || fail "v5.5.2 repository ZIP was not found."
REPO="$(find_repo || true)"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v552.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Installing v5.5.2 repository payload"
rsync -a --exclude='.git/' --exclude='.DS_Store' --exclude='__MACOSX/' --exclude='.venv/' --exclude='.pytest_cache/' --exclude='__pycache__/' "$SOURCE/" "$REPO/"

say "Running v5.5.2 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v552_validation.sh"

grep -Fq 'Version: 5.5.2' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Plugin version marker is not v5.5.2."
grep -Fq "SC_LIBRARY_VERSION', '5.5.2'" "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "SC_LIBRARY_VERSION is not v5.5.2."
grep -Fq "public const VERSION = '5.5.2'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-python-backend.php" || fail "Python backend bridge is not v5.5.2."
grep -Fq '__version__ = "1.0.2"' "$REPO/library-backend/app/__init__.py" || fail "Python backend service is not v1.0.2."
grep -Fq '127.0.0.1:8087:8080' "$REPO/library-backend/compose.yml" || fail "Backend compose file is not localhost-bound."
grep -Fq '/v1/admin/integrity' "$REPO/library-backend/app/main.py" || fail "Integrity API contract is missing."
