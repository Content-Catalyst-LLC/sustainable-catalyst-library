...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Field Spotlight Administration Console & Panel Curation Workflow"
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v436-validation-venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v436.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.6 release validation"
bash "$REPO/tests/run_v436_validation.sh"
...
printf 'Public Field Spotlight presentation remains the v4.3.5 Article Map hero + thumbnail supporting-card shell.\n'
