...
RELEASE_VERSION="4.3.22.4"
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Publications 14-Field Stack Restoration"
...
ensure_pytest(){
  ...
  venv="$DOWNLOADS/.sc-library-v432224-validation-venv"
  rm -rf "$venv"
  "$base_py" -m venv "$venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v432224.XXXXXX")"
trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.22.4 validation"
SC_LIBRARY_VALIDATION_PYTHON="$SC_LIBRARY_VALIDATION_PYTHON" bash "$REPO/tests/run_v432224_validation.sh"

grep -q 'Version: 4.3.22.4' "$REPO/sustainable-catalyst-library/sustainable-catalyst-library.php" || fail "Installed plugin version marker is not v4.3.22.4."
grep -q "public const VERSION = '4.3.22.4'" "$REPO/sustainable-catalyst-library/includes/class-sc-library-field-spotlights.php" || fail "Field Spotlight runtime was not upgraded to v4.3.22.4."
grep -q 'data-sc-field-stack="v4.3.22.4"' "$REPO/sustainable-catalyst-library/templates/field-spotlights.php" || fail "14-field stack marker is missing."
grep -q 'data-sc-field-stack-mode="all-fields"' "$REPO/sustainable-catalyst-library/templates/field-spotlights.php" || fail "All-fields stack mode is missing."
grep -q 'foreach ( \$field_list as \$index => \$stack_field )' "$REPO/sustainable-catalyst-library/templates/field-spotlights.php" || fail "Field stack does not iterate the complete field list."
grep -q 'sc-field-spotlights--stack-item' "$REPO/sustainable-catalyst-library/templates/field-spotlights.php" || fail "Independent stacked Field Spotlight roots are missing."
grep -q '\$include_data = true;' "$REPO/sustainable-catalyst-library/templates/field-spotlights.php" || fail "Independent field runtime data is not enabled."
if grep -q 'sc-field-spotlights__master-data' "$REPO/sustainable-catalyst-library/templates/field-spotlights.php"; then fail "Shared master-stage payload is still present in canonical stack template."; fi
grep -q 'new SC_Library_Field_Spotlights' "$REPO/sustainable-catalyst-library/includes/class-sc-library-publications.php" || fail "Legacy [sc_publications] canonical-page promotion is missing."
...
printf 'The Publications page body does not need replacement: both historical Publications shortcodes are promoted to the restored 14-field stack on the canonical route.\n'
