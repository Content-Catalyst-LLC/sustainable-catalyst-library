...
RELEASE_ZIP="${2:-$BUNDLE_DIR/sustainable-catalyst-library-v5.14.0-repository.zip}"
TAG="v5.14.0"
TMP="$(mktemp -d /tmp/sc-library-v5140.XXXXXX)"
trap 'rm -rf "$TMP"' EXIT
...
SOURCE_DIR="$TMP/sustainable-catalyst-library-v5.14.0"
[[ -f "$SOURCE_DIR/library-backend/app/citation_graph.py" ]] || fail "repository payload missing citation graph"
cd "$SOURCE_DIR"
./tests/run_v5140_validation.sh
echo "=== SYNCHRONIZING v5.14.0 INTO GIT CHECKOUT ==="
...
if git diff --cached --quiet; then
  ...
  git commit -m "Library v5.14.0 Citation Graph & Scholarly Lineage"
fi
if git rev-parse "$TAG" >/dev/null 2>&1; then
  target="$(git rev-list -n1 "$TAG")"; head="$(git rev-parse HEAD)"
  [[ "$target" == "$head" ]] || fail "$TAG already exists and does not point to HEAD"
else
  git tag -a "$TAG" -m "Sustainable Catalyst Library v5.14.0 — Citation Graph & Scholarly Lineage"
fi
git push origin HEAD
git push origin "$TAG"
echo "PASS: Knowledge Library v5.14.0 committed, tagged, and pushed."
