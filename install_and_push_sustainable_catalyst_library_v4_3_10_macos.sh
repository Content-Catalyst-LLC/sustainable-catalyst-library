...
RELEASE_NAME="Sustainable Catalyst Library v${RELEASE_VERSION} — Supporting Article Slot Persistence & Public Binding Repair"
...
ensure_pytest(){
  ...
  tool_venv="$DOWNLOADS/.sc-library-v4310-validation-venv"
...
TMP="$(mktemp -d "${TMPDIR:-/tmp}/sc-library-v4310.XXXXXX")"; trap 'rm -rf "$TMP"' EXIT
unzip -q "$ZIP" -d "$TMP"
...
say "Running v4.3.10 release validation"
bash "$REPO/tests/run_v4310_validation.sh"
...
printf 'Public Field Spotlights: supporting article selections now publish on save beneath the Article Map hero; v4.3.9 accordion, white active panel, sharp geometry, and eight-panel autoplay behavior are preserved.\n'
