...
ZIP="${1:-/tmp/sustainable-catalyst-library-backend-v1.7.0.zip}"
[[ -f "$ZIP" ]] || { echo "ERROR: backend ZIP not found: $ZIP" >&2; exit 1; }
[[ -f "$LIVE/.env" ]] || { echo "ERROR: existing backend .env not found at $LIVE/.env" >&2; exit 1; }
ts="$(date +%Y%m%d-%H%M%S)"
tmp="$(mktemp -d /tmp/sc-library-backend-v170.XXXXXX)"
trap 'rm -rf "$tmp"' EXIT
env_backup="/tmp/sc-library-backend-v1.7.0.env.$ts"
app_backup="$BASE/library-backend.before-v1.7.0-$ts"
...
grading_json="$(curl -fsS http://127.0.0.1:8087/v1/evidence-grading)"
printf '%s\n' "$health_json" | python3 -m json.tool
printf '%s\n' "$grading_json" | python3 -m json.tool
version="$(printf '%s' "$health_json" | python3 -c 'import json,sys; print(json.load(sys.stdin).get("version", ""))')"
[[ "$version" == "1.7.0" ]] || { echo "ERROR: expected backend v1.7.0, got: $version" >&2; exit 1; }
python3 -c 'import json,sys; d=json.load(sys.stdin); g=d.get("framework",{}).get("governance",{}); assert d.get("framework",{}).get("key")=="biomedical-evidence-grading"; assert g.get("formal_grade_generated") is False; assert g.get("formal_risk_of_bias_judgment_generated") is False' <<<"$grading_json"
printf '\nPASS - Library backend v1.7.0 upgraded; Biomedical Evidence Grading & Study Design Intelligence is active.\n'
